...
date: "2025-10-07"
lastmod: "2025-10-12T02:01:00.000Z"
draft: false
series: []
authors:
  - "Whitea"
...
NOTION_METADATA:
  ...
  last_edited_time: "2025-10-12T02:01:00.000Z"
  created_by:
    object: "user"
...
- Cloud 平台需要实现多租户的能力，实现基本的组织权限管理能力
- 打通控制面到数据面的权限控制，平台角色权限可直接映射到内核角色权限

**技术方案与实现**


Organzation 和 Project 表设计


基于Casbin实现，接口改造，统一过滤中间件，（扩展性）


新增创建数据库实例初始化脚本能力，初始化默认用户、角色并分配权限

1. **线上数据库版本升级**

编写版本升级Job，通过内置root连接数据库执行升级脚本，回滚策略

1. **Cloud 数据面备份恢复**

**需求与背景**

- 目前已经有基于物理机和虚拟机的备份恢复能力，需实现云上场景的备份恢复

**困难**

- 本地方案中通过 script/nebula.service start/stop 启停服务在云上场景不适用，容器 SigTerm 信号，需要
- 为了保证云数据库的存储低时延，选用 AWS 的 EBS 而非 EFS 作为存储，快存储只支持 ReadWriteOnly，需要考虑 sidecar container 方案
